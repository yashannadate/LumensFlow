...
export default function StreamCard({ stream, onAction }) {
  ...
  useEffect(() => {
    if (stream.status !== 'Active') {
      const total = (Number(stream.withdrawn_amount) + Number(stream.remaining_balance)) / 1e7
  ...
  const progress = Math.min(100, (withdrawable * 1e7 / (Number(stream.deposit_amount) - Number(stream.withdrawn_amount))) * 100 || 0)
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px', minWidth: '80px' }}>
        <div style={{ 
          ...
          background: stream.status === 'Active' ? 'rgba(34,197,94,0.12)' : 'rgba(148,163,184,0.08)',
          border: `1px solid ${stream.status === 'Active' ? 'rgba(34,197,94,0.3)' : 'rgba(148,163,184,0.2)'}`,
          color: stream.status === 'Active' ? '#22c55e' : '#94a3b8',
